    ...
    <div class="space-y-3">
      ...
      @if (!vistaDescartados()) {
        ...
        <div class="grid grid-cols-2 gap-2">
          <select (change)="store.orden.set(valor($event))" class="campo col-span-2 py-2.5 text-sm">
          ...
          <select (change)="store.cambiarDistrito(valor($event))" class="campo py-2.5 text-sm">
          ...
          <select (change)="store.fBarrio.set(valor($event))"
          ...
          <select (change)="store.fEstadoPiso.set(valor($event))" class="campo col-span-2 py-2.5 text-sm">
        ...
        <!-- ── Vista DESCARTADOS ── -->
        @if (store.pisosDescartados().length === 0) {
          <div class="tarjeta px-4 py-12 text-center">
            <p class="text-4xl">🗑️</p>
            <p class="mt-2 text-sm font-semibold text-text">Sin pisos descartados</p>
            <p class="mt-1 text-xs text-muted">Los pisos que descartes aparecerán aquí.</p>
          </div>
        } @else {
          <div class="grid gap-4 pt-1 sm:grid-cols-2 xl:grid-cols-3">
            @for (piso of store.pisosDescartados(); track piso.id) {
              <app-piso-card
                [piso]="piso"
                (editar)="editar.emit($event)"
...
export class ListaView {
  ...
  readonly vistaDescartados = signal(false);
